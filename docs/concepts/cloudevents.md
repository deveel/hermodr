...
- `source` must be set on the event itself _or_ via `EventPublisherOptions.Source`.
...
> Full payload validation (checking the `data` field against its declared schema) is deferred to a later milestone.  See [Schema Validation](../schema/validation.md) for the roadmap item.
