...
/// The in-process <c>OutboxRelayService</c> background service then picks them up
/// and forwards them to the RabbitMQ broker.
/// </summary>
public sealed class OrderManagementService : IOrderService
{
    ...
    public async Task<Order> CreateAsync(CreateOrderRequest request, CancellationToken ct = default)
    {
        ...
        // Publishing saves the event to the outbox (SQLite). The relay will pick it up
        // and forward it to RabbitMQ within the configured polling interval.
