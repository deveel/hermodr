...
### `Hermodr.Publisher.Http`

[![NuGet](https://img.shields.io/nuget/v/Hermodr.Publisher.Http.svg)](https://www.nuget.org/packages/Hermodr.Publisher.Http)
[![GitHub pre-release](https://img.shields.io/badge/nuget-prerelease-yellow?logo=nuget)](https://github.com/deveel/hermodr/pkgs/nuget/Hermodr.Publisher.Http)

A lightweight publish channel that delivers CloudEvents directly to statically-configured HTTP endpoints using the CloudEvents HTTP binding (structured or binary content mode). Fans a single publish call out to all endpoints concurrently via `IHttpClientFactory`, each with its own authentication (Bearer / API key), resilience (retry and circuit-breaker), and wire format. No subscriber registry, no signing, no delivery receipts — the right-sized complement to the Webhook publisher.
